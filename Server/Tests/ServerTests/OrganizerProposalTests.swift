...
@Suite("CoInstructorsWrapper Tests")
struct CoInstructorsWrapperTests {
  ...
  func roundTrip() throws {
    ...
    let decoded = try JSONDecoder().decode(CoInstructorsWrapper.self, from: data)

    #expect(decoded == wrapper)
    #expect(decoded.items.count == 2)
    #expect(decoded.items[0].name == "Alice")
    #expect(decoded.items[1].sns == "@bob")
  }
  ...
  @Test("decodes from a plain JSON array (existing DB data)")
  func decodesFromArray() throws {
    let json = """
      [{"name":"Eve","email":"eve@example.com","githubUsername":"eve","bio":"Bio E"}]
      """
    let data = Data(json.utf8)
    let wrapper = try JSONDecoder().decode(CoInstructorsWrapper.self, from: data)

    #expect(wrapper.items.count == 1)
    #expect(wrapper.items[0].name == "Eve")
  }
  ...
  @Test("Proposal.coInstructors computed property round-trips through wrapper")
  func proposalComputedProperty() {
    let proposal = Proposal()
